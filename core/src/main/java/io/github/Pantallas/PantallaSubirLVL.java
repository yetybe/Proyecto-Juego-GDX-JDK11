...
public class PantallaSubirLVL implements Screen {
    ...
    public void render(float delta) {	
        ...
        game.getFont().draw(game.getBatch(), "[1] Aumentar Daño", 400, 400);
        game.getFont().draw(game.getBatch(), "[2] Aumentar Vida Máxima", 400, 350);
        game.getFont().draw(game.getBatch(), "[3] Aumentar Velocidad", 400, 300);
        game.getFont().draw(game.getBatch(), "[4] Aumentar Vel. de Ataque", 400, 250);
        ...
        if (Gdx.input.isKeyJustPressed(Input.Keys.NUM_1)) {
            ...
            pjJugador.setDañoAtaque(pjJugador.getDañoAtaque() + 3.5f);
            volverAlJuego();
			...
			float nuevaCadencia = pjJugador.getCadenciaAtaque() - 0.08f;
	    
			if (nuevaCadencia < 0.03f) {
				nuevaCadencia = 0.03f;
			}
